...
class MAML(MetaTemplate):
    ...
    def set_forward(self, x, y=None):
        ...
        if y is None:  # Classification task, assign labels (class indices) based on n_way
            y_a_i = Variable(torch.from_numpy(np.repeat(range(self.n_way), self.n_support)).long())  # label for support data
        else:  # Regression task, keep labels as they are
            y_var = Variable(y)
            y_a_i = y_var[:, :self.n_support].contiguous().view(self.n_way * self.n_support,
                                                                *y.size()[2:]).long()  # label for support data
        if torch.cuda.is_available():
            y_a_i = y_a_i.cuda()
    ...
    def set_forward_loss(self, x, y=None):
        ...
        if y is None:  # Classification task
            y_b_i = Variable(torch.from_numpy(np.repeat(range(self.n_way), self.n_query)).long())
        else:  # Regression task
            y_var = Variable(y)
            y_b_i = y_var[:, self.n_support:].contiguous().view(self.n_way * self.n_query, *y.size()[2:]).long()
